...
if pos < syml
    disp(['pss_sync pos = ' num2str(pos)  ' incremented pos= ' num2str(pos+syml)]);
    pos=pos+syml;
end
...
%xcorr over 200 slots
for h=0:25 % (length(invec)/slot_symbs) % -25
    ...
    for j=0:6
        ...
        if idx >= length(invec(:,1))
            idx = length(invec(:,1));
        end
        
        
        ft1=fft(invec(idx-fftl+1:idx,1) ,fftl);
        t1=[ft1(fftl-30:fftl); ft1(1:32)];
        ft2=fft(invec(idx-fftl+1:idx,2) ,fftl);
        t2=[ft2(fftl-30:fftl); ft2(1:32)];
        
        
        
        c0x=xcorr(t1,chu0)+xcorr(t2,chu0);
        [c0 cpos(1)]=max(c0x);
        
        c1x=xcorr(t1,chu1)+xcorr(t2,chu1);
        [c1 cpos(2)]=max(c1x);
        
        c2x=xcorr(t1,chu2)+xcorr(t2,chu2);
        [c2 cpos(3)]=max(c2x);
        ...
        if abs(max(c))>abs(max_corr)
            ...
            title(['lte PSS sync Nid2=' num2str(N_id_2)])
            
            
            cmnorm=m/(sqrt(sum(abs(t1+t2))))*sqrt(62); %*sqrt(sum(abs(chu0))))
...
for m=max_pos-10:max_pos+10
    ft1=fft(invec(m-fftl+1:m,1) ,fftl);
    t1=[ft1(fftl-30:fftl);ft1(1:32)];
    ft2=fft(invec(m-fftl+1:m,2) ,fftl);
    t2=[ft2(fftl-30:fftl);ft2(1:32)];
    
    c1x=xcorr(t1,chu_n)+xcorr(t2,chu_n);
    c1=max(c1x);
...
disp(['max mag1 = ' num2str(abs(maxmax(1))) ' relpos1 = ' num2str(relpos(1))]);
subplot(2,2,3);
plot(abs(corrvec(:,1)));
title('lte PSS fine sync')
...
chu_s1=invec(max_pos-fftl+1:max_pos,1);
fine_corr=xcorr(chu_s1,chu_t);
chu_s1=invec(max_pos-fftl+1:max_pos,2);
fine_corr=fine_corr+xcorr(chu_s1,chu_t);

if length(invec(:,1)) > max_pos+10*slot_symbs
    chu_s2=invec(max_pos-fftl+1+5*slot_symbs:max_pos+5*slot_symbs,1);
    fine_corr=(fine_corr+xcorr(chu_s2,chu_t));
    chu_s2=invec(max_pos-fftl+1+5*slot_symbs:max_pos+5*slot_symbs,2);
    fine_corr=(fine_corr+xcorr(chu_s2,chu_t));
end
if length(invec(:,1)) > max_pos+20*slot_symbs
    chu_s3=invec(max_pos-fftl+1+10*slot_symbs:max_pos+10*slot_symbs,1);
    fine_corr=(fine_corr+xcorr(chu_s3,chu_t));
    chu_s3=invec(max_pos-fftl+1+10*slot_symbs:max_pos+10*slot_symbs,2);
    fine_corr=(fine_corr+xcorr(chu_s3,chu_t));
end
...
disp(['PSS fine_pos= ' num2str(max_pos) ' coarse_pos = ' num2str(save_max_pos) ' diff = ' num2str(max_pos-save_max_pos)]);
...
pss_pos=max_pos; % -2; % -2, because early sync only results in a phase shift. otherwise ISI expected.
