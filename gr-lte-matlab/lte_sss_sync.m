...
disp(['sss_pos1 = ' num2str(sss_pos+1)]);
sss_t=invec(sss_pos+1:sss_pos+fftl,:);
sss_f=fft(sss_t,fftl);
sss_f=circshift(sss_f,-ni);
...
abs_corr(:,1)=abs(xcorr(sref,s0m0(:,1)))+abs(xcorr(sref,s0m0(:,2)));
...
abs_corr(:,1)=abs(xcorr(sref,s1m1(:,1)))+abs(xcorr(sref,s1m1(:,2)));
